...
class Controller(object):
    def __init__(self, config):
        ...
        self.users = [User(n, c) for (n, c) in config['users'].items()]
        ...
        elif self.alert_type == "twilio":
            self.account_sid = config['alerts']['twilio']['account_sid']
            self.auth_token = config['alerts']['twilio']['auth_token']
        else:
            ...
            if new_state == 'open' and not door.msg_sent and time.time() - door.open_time >= self.ttw:
                if self.use_alerts:
                    ...
                    if self.alert_type == 'smtp':
                        ...
                        self.send_twilio_sms(door, title, message)
                    door.msg_sent = True
    ...
    def send_twilio_sms(self, door, title, message):
        syslog.syslog("Sending Twilio SMS message")
        config = self.config['alerts']['twilio']
        ...
        for user in self.users:
            ...
            try:
                send = client.api.account.messages.create(to=user.phone_number, from_="+18329253204", body=message)
    ...
    def run(self):
        task.LoopingCall(self.status_check).start(0.25)
        root = File('www')
        ...
        #/etc/letsencrypt/live/joe.vpham.com/fullchain.pem
        contextFactory = certificateOptionsFromFiles(
            '/etc/letsencrypt/live/joe.vpham.com/privkey.pem',
            '/etc/letsencrypt/live/joe.vpham.com/fullchain.pem')
    ...
    def decode_data(self, encoded_data):
        if encoded_data is not None:
            data = encoded_data[0]
            decoded_data = jwt.decode(data, 'altamira')
            return decoded_data
        else:
            return None
...
class ClickHandler(Resource):
    ...
    def render(self, request):
        decoded_data = self.controller.decode_data(request.args.get('data', None))
        door = decoded_data.get('id', None)
        uuid = decoded_data.get('uuid', None)
        if self.controller.authenticate(uuid):
